...
def asr_and_summary_background_task(audio_id: str, language: str, diarization: bool = True, hotwords: list = []):
    """
    Background task to process audio: ASR + Summarization
    """
    db = SessionLocal()
    try:
        ...
        # Nếu API lỗi hoặc return None
        if run_request is None or "transcription" not in run_request:
            logger.error("ASR service failed - no transcription returned")
            audio.status = "failed"
            db.commit()
            return False
        ...
        if run_request_sum is None or "summary" not in run_request_sum:
            logger.error("Summarization service failed - no summary returned")
            audio.status = "failed"
            db.commit()
            return False
        ...
        db.commit()
        logger.info(f"[SUCCESS] ASR+Summary completed for audio_id={audio_id}")
        return True
